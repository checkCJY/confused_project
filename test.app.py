...
with st.expander("📝 새 거래 등록", expanded=True):
    ...
    if st.button("등록"):
        if amount > 0 and content.strip():
            ...
            st.toast("✅ 등록 완료!"); time.sleep(1); st.rerun()

if not st.session_state.history:
    st.info("데이터를 등록해주세요."); st.stop()

# [3] 데이터 가공 및 필터링
df_all = logic.process_dataframe(st.session_state.history)
st.divider()
f_c1, f_c2 = st.columns(2)
date_range = f_c1.date_input("기간", [df_all["date"].min(), df_all["date"].max()])
keyword = f_c2.text_input("검색어")
...
with tab1:
    st.dataframe(filter_df.sort_values("date", ascending=False), use_container_width=True)
with tab2:
