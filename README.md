...
✅D1 기간필터 
✔️D2 메모검색 (메모로만 검색가능. 아쉬움)
✔️D3 간단한 테스트 코드 
- 예상 가능한 부분만 작성
- 추가로 코드에서 try-execpt를 구현못해서 아쉬움
✔️D4 지출 한도 알림
- 구현은 어느정도 되었으나, 그래프에서 아쉬움 
- 예산에서 줄어드는식 이면 좋을 것 같다
```

### 구현해보고 싶은 기능
```
✔️ 카테고리와 내용이 일치하는, 검색시 카테고리 또는 내용 으로 적용
✔️ 데이터파일 백업 구현은 했으나, 원본 데이터가 이름이 바뀌는 부분이 있어 삭제
✔️ 위와 관련하여 데이터 파일을 업로드 하고, 그 파일을 기준으로 접근하는게 어려웠다.
```
